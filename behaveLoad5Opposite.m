...
function [taskbase] = behaveLoad5Opposite(fileDirectory)
...
% fileDirectory = '/Users/stubblefielde/Desktop/behaviorFebMarApr16/Vgatseven';
...
CLvalidRew = time_thCL(CLvalid);                                                   % centering TS for L-bar corrects

% R wheel movement extractions for correct L-bar trials
wheelCL = {};
wheelCLtime = {};
wheelLmove = {};
wheelLmoveTime = {};
                ...
                if i+1 == length(barpositionCell{z})
                    break
                        ...
                        wheelLmoveTime{z}(i) = NaN;                             % prevent repeating time(positions) from being zero
                            ...
                            wheelCL{z}(i) = barpositionCell{z}(i);              % wheel positions during rightward movements
                            wheelCLtime{z}(i) = time_trial{z}(i);               % all TS of lrightward movements
                            wheelLmove{z}(i) = barpositionCell{z}(i+1);         % bit of a hack for the FIRST R movement 
                            wheelLmoveTime{z}(i) = time_trial{z}(i+1);          % & thus, hack for the FIRST TS of R movement
                        end
                    end
                end
            end
...
% initial L-trial bar/wheel movements that lead to centering for L trials
    ...
    if numel(find(wheelCL{c} >= 0) > 0)
        for i=1:length(wheelCLtime{c})
            ...
            if wheelLmove{c}(i) > posStartsTrueMaxNeg && wheelLmove{c}(i) < wheelLmove{c}(i+1) && wheelLmove{c}(i) < 0 % still may need to correct for large movements that began < -200
                wheelLfirst(c) = wheelLmove{c}(i);                          % first bar position for R movements 
                wheelLfirstTime(c) = wheelLmoveTime{c}(i);                  % first TS of R movement vectors
...
if numel(trialEndTimes) < numel(trialStartTimes)
    trialStartTimesNew = trialStartTimes(1:end-1);
end
...
taskbase.trainingDay = trainingDay;
