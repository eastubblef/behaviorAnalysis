...
function [taskbase] = behaveLoad(fileDirectory)
...
for c = 1:length(barpositionCell)
    if numel(find(wheelCLtime{c} >= 0) > 0)
        for i=1:length(wheelCLtime{c})
            for p = 1:length(CLvalidRew)
                if i == length(wheelCLtime{c})
                    break
                end
                if i == 1 && wheelCLtime{c}(i) == CLvalidRew(p)             % in case centering occurred on the 2nd, 3rd value, etc
                    wheelCLtimeValid(p) = wheelCLtime{c}(i);                % gotta be an easier way to do this; plus, I'm missing tail end centerings
                else if i == 2 && wheelCLtime{c}(i) == CLvalidRew(p)
                        wheelCLtimeValid(p) = wheelCLtime{c}(i-1);
                    else if i == 3 && wheelCLtime{c}(i) == CLvalidRew(p)
                            wheelCLtimeValid(p) = wheelCLtime{c}(i-2);
                        else if i == 4 && wheelCLtime{c}(i) == CLvalidRew(p)
                                wheelCLtimeValid(p) = wheelCLtime{c}(i-3);
                            else if i == 5 && wheelCLtime{c}(i) == CLvalidRew(p)
                                    wheelCLtimeValid(p) = wheelCLtime{c}(i-4);
                                else if i == 6 && wheelCLtime{c}(i) == CLvalidRew(p)
                                        wheelCLtimeValid(p) = wheelCLtime{c}(i-5);
                                        
                                    else if wheelCLtime{c}(i) == CLvalidRew(p) && wheelCLtime{c}(i-1) > 1 && wheelCLtime{c}(i-2) == 0
                                            wheelCLtimeValid(p) = wheelCLtime{c}(i-1);
                                            
                                        else  if wheelCLtime{c}(i) == CLvalidRew(p) && wheelCLtime{c}(i-2) > 1 && wheelCLtime{c}(i-3) == 0
                                                wheelCLtimeValid(p) = wheelCLtime{c}(i-2);
                                                
                                            else if wheelCLtime{c}(i) == CLvalidRew(p) && wheelCLtime{c}(i-3) > 1 && wheelCLtime{c}(i-4) == 0
                                                    wheelCLtimeValid(p) = wheelCLtime{c}(i-3);
                                                    
                                                else if wheelCLtime{c}(i) == CLvalidRew(p) && wheelCLtime{c}(i-4) > 1 && wheelCLtime{c}(i-5) == 0
                                                        wheelCLtimeValid(p) = wheelCLtime{c}(i-4);
                                                        
                                                    else if wheelCLtime{c}(i) == CLvalidRew(p) && wheelCLtime{c}(i-5) > 1 && wheelCLtime{c}(i-6) == 0
                                                            wheelCLtimeValid(p) = wheelCLtime{c}(i-5);
                                                        end
                                                        %                 else if wheelCLtime{c}(i) == CLvalidRew(p) && wheelCLtime{c}(i-1) > 1
                                                        %                         wheelCLtimeValid(p) = wheelCLtime{c}(i-1);          % fix this to include first L movement
                                                        %                 end
...
%                     break
%                 end
%                 if thCLnew(p) <= 2 && wheelCL{c}(i) > 1 && i == 2
%                     wheelCLtimeFind(p) = wheelCL{c}(i);                               % in case centering occurred by the 2nd value 
%                    
%                 if thCLnew(p) > 2 && wheelCL{c}(i) < 0
%                         wheelCLtimeFind(p) = min(wheelCL{c} > posStartsTrueMaxNeg); % should find min just > -200
%                     end
%                     
%                 end
%             end
