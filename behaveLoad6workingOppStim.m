...
function [taskbase] = behaveLoad6workingOppStim(fileDirectory, removeLongTrials, excludeTrials)

% 06.15.17 - old rx time bins:  <200; 200-600; 600-1000; >1000 ms
...
for i = 1:length(wheelLfirstTime)
    if i == numel(wheelLfirstTime)
        break
    else if rxCondensed3(i) == -1 && rx(i) < 200 && rxCondensed4(i) == 0
...
for j = 1:length(wheelRfirstTime)
    if j == numel(wheelRfirstTime)
        break
    else if rxCondensed3(j) == 1 && rx(j) < 200 && rxCondensed4(i) == 0
...
rxSortFastest = noStimMat2(fastestInds);
% corVecFastest = mu(fastestInds);
corVecFastest = noStimMat3(fastestInds);
...
rxSortFast = noStimMat2(fastInds);
% corVecFast = mu(rxSort1 > edges(2) & rxSort1< binwidthFast);
corVecFast = noStimMat3(noStimMat2 > edges(2) & noStimMat2 < binwidthFast);
...
midInds = find(noStimMat2 > edges(4) & noStimMat2 < binwidthMid);
...
corVecMid = noStimMat3(noStimMat2 > edges(4) & noStimMat2 < binwidthMid);  
...
slowInds = find(noStimMat2 > binwidthMid);
% rxSortSlow = rxSort1(slowInds);                         %rx times for the mid-range bin (600-1000 ms)
rxSortSlow = noStimMat2(slowInds);
% corVecSlow = mu(rxSort1 > binwidthMid);
corVecSlow = noStimMat3(noStimMat2 > binwidthMid);
...
stimMat2 = stimMat(:,2);
stimMat3 = stimMat(:,3);
fastestIndsStim = find(stimMat(:,2) < binwidthFastest);  
...
binwidthFast = edges(4) - edges(1);
fastIndsStim = find(stimMat2 > edges(2) & stimMat2 < binwidthFast);
rxSortFastStim = stimMat2(fastIndsStim);
corVecFastStim = stimMat3(stimMat2 > edges(2) & stimMat2 < binwidthFast);
...
binwidthMid = edges(6);
midIndsStim = find(stimMat2 > edges(4) & stimMat2 < binwidthMid);
rxSortMidStim = stimMat2(midIndsStim);
corVecMidStim = stimMat3(stimMat2 > edges(4) & stimMat2 < binwidthMid);  
...
slowIndsStim = find(stimMat2 > binwidthMid);
rxSortSlowStim = stimMat2(slowIndsStim);
corVecSlowStim = stimMat3(stimMat2 > binwidthMid);
...
histRx1stim = histc(rxSortFastestStim, edges);                   % 0-200 ms bin
...
h1stim = bar(edges, histRx1stim, 'r'); set(h1, 'BarWidth', 1);  % plot last for visual purposes

    xlabel('Rx times (ms), 200ms bins'); ylabel('# Trials');
    set(gca, 'Xlim', [0 5200]);
    set(gca, 'Xtick', [0:500:5200]), 
    num200 = num2str(round(NumFastestStim)); num200_600 = num2str(round(NumFastStim)); num600_1000 = num2str(round(NumMidStim)); num1000 = num2str(round(NumSlowStim));
    legend(num1000,num600_1000,num200_600,num200,'Location','northeast');
