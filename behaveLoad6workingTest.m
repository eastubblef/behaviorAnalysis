...
% fileDirectory = '/Users/stubblefielde/Desktop/mfiles/DudmanLab/data/spikes/recordings16/161005/behaveChunks';
...
%                 if isempty(time_trial{n}(i)) 
%                     find_thILtimes{n}(i) = time_trial{n}(i-1);                    % adding NaN above makes these lines obselete
%                 else
%                    find_thILtimes{n}(i) = time_trial{n}(i);
%                 end               
%                 if i > 1 && isnan(time_trial{n}(end))                             % in case timestamp wasn't collected (160516 and test)
                ...
                for i=1:length(barpositionCell{z})
                    ...
                    if i+1 == length(barpositionCell{z})
                        break
                            ...
                            %                     else if barpositionCell{z}(i) > barpositionCell{z}(i+1)                                  % moving in incorrect direction - move this up
                            %                         wheelIL{z}(i) = barpositionCell{z}(i);               % wheel positions during incorrect L trials
                            %                         wheelILtime{z}(i) = time_trial{z}(i);                % all TS of incorrect L trials
                            %                         wheelILmove{z}(i) = barpositionCell{z}(i+1);         % bit of a hack for the FIRST IL movement - Need to change non-position zeroes to NaNs
                            %                         wheelILmoveTime{z}(i) = time_trial{z}(i+1);          % & thus, hack for the FIRST TS of IL movement
...
%             else if wheelILmove{c}(i) < posStartsTrueMaxNeg && wheelILmove{c}(i) > wheelILmove{c}(i+1) && wheelILmove{c}(i) < 0     % nope, subsequent vals became zero, above
        ...
        if length(find(barpositionCell{n} >= posStartsTrueMaxIR) > 0);      % if bar goes off to right on right trial
            for i=1:length(barpositionCell{n})
                ...
                if barpositionCell{n}(i) < 0                                % eliminate sneaky correct R trials
                    ...
                    time_thIR(n) = NaN;
                    break
                    %                 else find_thIR(n) = barpositionCell{n}(i);
                end
...
%     if numel(find(wheelIRmove{c} >= posStartsTrueMaxIR) > 0)                 % this line was skipping what it needed to find
...
% taskbase.rewTimes = rewTimes(2:end);     % correct for zeros at first index
