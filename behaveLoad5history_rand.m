...
% function [taskbase] = behaveLoad5history_rand(fileDirectory)
function [Prob_Li, Prob_LiMinusOne1, Prob_LiMinusTwo1, Prob_LiMinusThree1, Prob_LiMinusFour1, Prob_LiMinusFive1, Prob_LiMinusSix1, Prob_LiMinusSeven1] = behaveLoad5history_rand(fileDirectory)
...
%8.11.16: blocked trials ex = Vgatsix from 7.29.16
%8.12.16: Also holds up for Vgatsix 7.28.16 and 7.12.16 sessions
...
trialConstrainMax = 97; %best performance within 100ish trials
...
% fileDirectory = '/Users/stubblefielde/Desktop/mfiles/DudmanLab/data/spikes/recordings15/Vgattwo/151118/behaveChunks';
% fileDirectory = '/Users/stubblefielde/Desktop/behaviorOctNov15/VgatthreeRand';
...
% figure; 
% hold on; 
% plot(LRtrialStruct, 'k');
% plot(choiceVec, 'b');
%     
% ylabel('actual LR trial delivered');  
% % xlim([0 250]);
% xlim([60 170])
% xlabel('trial number'); 
% ylim([-2 2]);
...
% % totLchoiceTrialsConstrained1 = find(trialConstrainMax <= totLchoiceTrials >= trialConstrainMin);
% totLchoiceTrialsConstrained1 = find(totLchoiceTrials >= trialConstrainMin);   %works, but is not informative
% % totLchoiceTrialsConstrained2 = find(totLchoiceTrials <= trialConstrainMax);
% totLchoiceTrialsConstrained = totLchoiceTrials(totLchoiceTrialsConstrained1);
...
% Perc_Li = numProbL/Li * 100;                          %This will be the first x value on %(L) at i
Prob_Li1 = numProbL/Li;                                  %This will be the first x value on P(Li) at i
Prob_Li = Prob_Li1/Prob_Li1   %by definition, should be 1
...
% Perc_LiMinusOne = numel(numProbLminusOne)/(Li-1) * 100;  %Must divide by true numel (i=2:end); %This will be the second x value on P(Li) at i-1
Prob_LiMinusOne1 = numel(numProbLminusOne)/(Li-1);
Prob_LiMinusOne = Prob_LiMinusOne1/Prob_Li1;                  %by definition, divide by prob(Li)
...
% Perc_LiMinusTwo = numel(numProbLminusTwo)/(Li-2) * 100;
Prob_LiMinusTwo1 = numel(numProbLminusTwo)/(Li-2);
Prob_LiMinusTwo = Prob_LiMinusTwo1/Prob_Li1;                %by definition, divide by prob(Li)
...
% Perc_LiMinusThree = numel(numProbLminusThree)/(Li-3) * 100;
Prob_LiMinusThree1 = numel(numProbLminusThree)/(Li-3);
Prob_LiMinusThree = Prob_LiMinusThree1/Prob_Li1;                %by definition, divide by prob(Li)
...
% Perc_LiMinusFour = numel(numProbLminusFour)/(Li-4) * 100;
Prob_LiMinusFour1 = numel(numProbLminusFour)/(Li-4);
Prob_LiMinusFour = Prob_LiMinusFour1/Prob_Li1;  
...
% Perc_LiMinusFive = numel(numProbLminusFive)/(Li-5) * 100;
Prob_LiMinusFive1 = numel(numProbLminusFive)/(Li-5);
Prob_LiMinusFive = Prob_LiMinusFive1/Prob_Li1; 
...
% Perc_LiMinusSix = numel(numProbLminusSix)/(Li-6) * 100;
Prob_LiMinusSix1 = numel(numProbLminusSix)/(Li-6);
Prob_LiMinusSix = Prob_LiMinusSix1/Prob_Li1; 
...
% Perc_LiMinusSeven = numel(numProbLminusSeven)/(Li-7) * 100;
Prob_LiMinusSeven1 = numel(numProbLminusSeven)/(Li-7);
Prob_LiMinusSeven = Prob_LiMinusSeven1/Prob_Li1; 
...
% %     y = [Prob_Li, Prob_Li];
% %     ylabel('P choice (Left)', 'FontSize', 20);  %Just L choice; not L choice on i
% % 
% % %     y = [Prob_Li, Prob_Li, Prob_Li, Prob_Li, Prob_Li, Prob_Li, Prob_Li, Prob_Li]; %should get at (Prob choice Li) for Y axis
% %     x = [Prob_Li, Prob_LiMinusOne];
%      h = plot(y, 'r.', 'MarkerSize', 18);
% %     
% 
%     ylabel('P choice (Left_i)', 'FontSize', 20);  
%     xlabel('Trial history (i-num trials back) for 1 session', 'FontSize', 20); 
%     ylim([0 80]);
%     xlabels = single([0 1 2 3 4 5 6 7]);
%     xlabels1 = sort(xlabels, 'descend'); 
%     xlabels1 = single([i i-1 i-2 i-3 i-4 i-5 i-6 i-7]);    
% %     xlabels1 = strcat('i', 'i-1', 'i-2, 'i-3', 'i-4', 'i-5', 'i-6', 'i-7');
% %     xlabels = single([i i-1 i-2 i-3 i-4 i-5 i-6 i-7]);
% 
% %     ax = gca;
% %     set(ax);
% %     XTickLabelMode('manual');
% %     xlabels = {i i-1 i-2 i-3 i-4 i-5 i-6 i-7};
% 
% %     set(gca, 'XTick', xlabels1); Get this to work!
...
% hold on; 
% %     y = [Prob_Li, Prob_LiMinusOne, Prob_LiMinusTwo, Prob_LiMinusThree, Prob_LiMinusFour, Prob_LiMinusFive, Prob_LiMinusSix, Prob_LiMinusSeven];
% y = [Prob_Li, Prob_LiMinusOne1, Prob_LiMinusTwo1, Prob_LiMinusThree1, Prob_LiMinusFour1, Prob_LiMinusFive1, Prob_LiMinusSix1, Prob_LiMinusSeven1];        
% 
% %     y = [Prob_Li, Prob_Li, Prob_Li, Prob_Li, Prob_Li, Prob_Li, Prob_Li, Prob_Li]; %should get at (Prob choice Li) for Y axis
