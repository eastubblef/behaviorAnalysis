...
% file = 'mouseBehavior3.xls';        %updated 3.5.18
file = 'mouseBehavior4.xls';        %updated 3.26.18
...
% timespan1two = 428:596;               %half bar up to recording day2 - wasworking 3.6.18; updataed 
...
performMat = NaN(length(performMat4avg3), 3);
...
figure; hold on;
    ax = gca;
    mn = min(percCorrectsTotMat2two(:,1)); mx = max(percCorrectsTotMat2two(:,1));
    y1 = 75; y2 = 75;
...
plot(performMat2two(:,3), performMat2two(:,2), 'bl', 'LineWidth', 2);
...
percCorrectsTot6 = num(timespan6, 18);
...
%     mn = min(performMat6(:,1)); 
    mx = max(percCorrectsTotMat2two(:,1));
    y1 = 75; y2 = 75;
    l = line([0 mx], [y1 y2], 'Color', 'k'); % plot line first so that data points occlude it
    set(l,'LineStyle','--')
