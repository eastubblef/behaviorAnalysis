...
file = 'mouseBehavior4.xls';        %updated 3.5.18
...
plot(performRandAvg, 'k.', 'MarkerSize', 22);
...
performMatBlk = NaN(length(performMat4avg4), 3);
...
% performRandAvg = nanmean(performMat,2);
plot(performRandAvg, 'k', 'lineWidth', 4);
ylabel('Mean reaction time (ms)', 'FontSize', 28); ylim([0 6000]);
xlabel('Training day', 'FontSize', 28); xlim([0 80]);
ax = gca; 
ax.FontSize = 28;
